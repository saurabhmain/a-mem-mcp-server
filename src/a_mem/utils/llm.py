...
Unterstützt Ollama (lokal) und OpenRouter (Cloud) für LLM und Embeddings.
"""
...
class LLMService:
    ...
    def _call_llm(self, prompt: str, system: Optional[str] = None) -> str:
        """Ruft LLM auf (Provider-abhängig)."""
    ...
    def get_embedding(self, text: str) -> List[float]:
        """Berechnet Embedding (Provider-abhängig)."""
        if self.provider == "openrouter":
            try:
    ...
    def extract_metadata(self, content: str) -> dict:
        """Extrahiert Metadaten (Summary, Keywords, Tags) über LLM (Provider-abhängig)."""
...
Return ONLY the JSON object, no markdown formatting, no explanations."""
    ...
    def check_link(self, note_a: AtomicNote, note_b: AtomicNote) -> Tuple[bool, Optional[NoteRelation]]:
        """Prüft ob zwei Notes verknüpft werden sollen."""
...
Return ONLY the JSON object, no markdown formatting, no explanations."""
    ...
    def evolve_memory(self, new_note: AtomicNote, existing_note: AtomicNote) -> Optional[AtomicNote]:
        ...
        Prüft ob existing_note basierend auf new_note aktualisiert werden soll.
        Formel: mj* = LLM(mn || M_near || mj || Ps3)
        """
...
Return ONLY the JSON object, no markdown formatting, no explanations."""
        ...
        try:
            ...
            if data.get("should_update", False):
                # Erstelle aktualisierte Note mit neuer Information
                evolved_note = AtomicNote(
                    id=existing_note.id,  # ID bleibt gleich
                    content=existing_note.content,  # Content bleibt gleich (nur Metadaten werden verfeinert)
